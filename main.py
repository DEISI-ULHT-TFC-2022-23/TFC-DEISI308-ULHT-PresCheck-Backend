...
from flask import Blueprint, request, abort, redirect, render_template, url_for, flash
...
def iniciar_aula():
    ...
    unidades_associadas = UnidadeCurricular.query.filter_by(professor_id=current_user.professor_id)

    if request.method == "POST":
        sala_a_abrir = request.form.get('sala')
        # Verifica se a sala já está registrada e ativa nas aulas em andamento
        if sala_a_abrir in aulas_a_decorrer:
            flash('A sala já está registrada e ativa!', 'error')
            # Redireciona de volta para a página de início de aula com as unidades curriculares associadas
            return redirect(url_for('main.iniciar_aula', unidades=unidades_associadas))
    ...
    return render_template("iniciaraulaform.html", unidades=unidades_associadas)
...
def controlar_aula():
    if request.method == "POST":
        ...
        return render_template("controlaraula.html", aula=aulas_a_decorrer[sala])

    # Renderiza a página de controlo de aula sem dados da sala em andamento
    return render_template("controlaraula.html")
...
def exportar_presencas():
    ...
    # variáveis
    return render_template("exportarpresencas.html", aula=aula_selecionada, presencas=presencas_aula)
...
def registar_aluno():
    ...
    return render_template("registaraluno.html")
